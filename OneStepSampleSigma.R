...
#' @param mu An array of K 'p x 1' vectors denoting the column means of a cluster.
#' @param N A 'K x 1' vector denoting the counts in each cluster
#' @param K A scalar denoting the number of clusters specified
#' @return An array of matrices sampled from the inverse-wishart distribution where the kth matrix
...
OneStepSampleSigma <- function(theta, mu,N,K){
  ...
  nu <- N - p - 1
  Psi <- theta_mu %*% theta_mu # get scale parameter
